...
# --- RUTAS MODERNAS PARA AÑADIR ITEMS DESDE MODALES (USANDO JSON) ---
...
def agregar_experiencia():
    ...
    if not data or not data.get('cargo') or not data.get('empresa'):
...
def agregar_educacion():
    ...
    data = request.get_json()
    if not data or not data.get('institucion') or not data.get('titulo'):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'status': 'success', 'message': 'Formación añadida con éxito'}), 201
    except Exception as e:
        db.session.rollback()
        print(f"Error al guardar educación en DB: {e}")
        return jsonify({'status': 'error', 'message': 'Ocurrió un error interno al guardar los datos'}), 500
...
def agregar_proyecto():
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'status': 'success', 'message': 'Proyecto añadido con éxito'}), 201
    except Exception as e:
        db.session.rollback()
        print(f"Error al guardar proyecto en DB: {e}")
        return jsonify({'status': 'error', 'message': 'Ocurrió un error interno al guardar los datos'}), 500
